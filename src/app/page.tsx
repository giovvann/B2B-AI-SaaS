...
const overlay = typeof document !== 'undefined' ? document.getElementById('mobileOverlay') : null;
  ...
  useEffect(() => {
    // OAuth recovery: if Supabase redirected to the root with ?code (e.g. fallback to Site URL
    // instead of /auth/callback), forward to the callback route. If we landed on a different
    // host (preview deployment), bounce to the canonical site first so the PKCE verifier matches.
    const oauthParams = new URLSearchParams(window.location.search);
    if (oauthParams.has('code') || oauthParams.has('error')) {
      const canonical = (process.env.NEXT_PUBLIC_SITE_URL || 'https://velioralat.vercel.app').replace(/^https?:\/\//, '').split('/')[0];
      const host = window.location.hostname;
      const isLocal = host === 'localhost' || host.startsWith('127.');
      const base = isLocal || host === canonical ? '/auth/callback' : (process.env.NEXT_PUBLIC_SITE_URL || 'https://velioralat.vercel.app');
      window.location.replace(base + '?' + oauthParams.toString());
      return;
    }
...
<section className="video">
  <div className="container">
    <div className="video-inner r">
      ...
      <p style={{"fontSize":".82rem","color":"rgba(42,36,32,.6)","maxWidth":"480px","margin":"0 auto"}}>Sin buscar entre montones. Sin dejar a nadie esperando.</p>
  ...
  <div className="container">
    <div className="pricing-header r">
      ...
      <div className="billing-toggle" role="group" aria-label="Periodo de facturación">
        <button type="button" className={billing === 'monthly' ? 'active' : ''} onClick={() => setBilling('monthly')} aria-pressed={billing === 'monthly'}>Mensual</button>
        <button type="button" className={billing === 'annual' ? 'active' : ''} onClick={() => setBilling('annual')} aria-pressed={billing === 'annual'}>Anual <span className="save-badge">-20%</span></button>
      </div>
    </div>
    <div className="pricing-grid">
      ...
      <div className="pricing-card featured r d2">
        ...
        <div className="pricing-price" key={billing}>
          <span className="old">{billing === 'annual' ? '$199' : '$299'}</span>
        ...
        <a href={billing === 'annual' ? '/registro?trial=true&billing=annual' : '/registro?trial=true'} className="btn btn-primary" style={{"fontSize":".78rem"}}>
      ...
      <div className="pricing-card r d3">
        ...
        <div className="pricing-price" key={billing}>
          {billing === 'annual' ? <span className="old">$339</span> : null}
        ...
        <a href={billing === 'annual' ? '/registro?trial=true&billing=annual' : '/registro?trial=true'} className="btn btn-secondary" style={{"fontSize":".75rem"}}>Elegir Ilimitado</a>
      </div>
    </div>
  </div>
</section>
        ...
        <div id="downloadModal" onClick={(e) => e.stopPropagation()} className="bg-white rounded-2xl shadow-2xl max-w-sm w-full mx-4 p-6 scale-95 transition-transform duration-300">
          <div className="text-center">
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
              ...
              </div><div className="faq-item"><div className="faq-q" onClick={(e) => { tF(e.currentTarget) }}>Puedo tener varios empleados usando Veliora al mismo tiempo?<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M6 9l6 6 6-6"/></svg></div><div className="faq-a"><p>Claro. Cada empleado tiene su propio acceso con roles personalizados. Tu como dueña ves todo; ellos solo lo que necesitan para vender. Ideal si tienes varias personas en tu boutique.</p></div></div><div className="faq-item"><div className="faq-q" onClick={(e) => { tF(e.currentTarget) }}>Qué pasa con mi información si dejo de pagar?<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M6 9l6 6 6-6"/></svg></div><div className="faq-a"><p>Nada. Tus datos se quedan guardados y seguros. Si vuelves a activar tu membresía, todo está como lo dejaste. Además, puedes exportar tu información cuando quieras, sin restricciones.</p></div></div>
            </div>
